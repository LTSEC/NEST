...
// GameWorkDir returns the per-game terraform working directory.
...
// EnsureGameDir creates the per-game terraform directory structure and copies
...
// CleanupGameDir removes a per-game terraform directory after destruction.
...
// outputDir specifies where the generated main.tf should be written.
func GenerateTerraform(inputFile string) error {
	tfparser := filepath.Join("scripts", "tfparser.py")
	cmd := exec.Command("python", tfparser, "-i", inputFile)
...
// GenerateTerraformToDir generates terraform config and writes main.tf into
// the given output directory.
func GenerateTerraformToDir(inputFile, outputDir string) error {
	tfparser := filepath.Join("scripts", "tfparser.py")
	cmd := exec.Command("python", tfparser, "-i", inputFile)
	var out bytes.Buffer
	var stderr bytes.Buffer
	...
	// Write the generated terraform to the output directory
	mainTF := filepath.Join(outputDir, "main.tf")
	if err := os.WriteFile(mainTF, out.Bytes(), 0o644); err != nil {
