    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="mx-auto max-w-5xl space-y-6 p-6">
        <header className="space-y-1">
          <p className="text-xs font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Archives</p>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Archived games</h1>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {archives.map((archive) => (
            <article
              key={archive.id}
              className="flex flex-col gap-3 rounded-xl bg-white dark:bg-slate-900 p-4 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{archive.sessionName}</p>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">{archive.networkSummary ?? 'Network summary pending.'}</p>
              <div className="flex items-center justify-between text-[11px] font-semibold text-slate-700 dark:text-slate-300">
                <span>{archive.results.length} teams captured</span>
                <button
                  type="button"
                  onClick={() => navigate(`/archives/${archive.id}`)}
                  className="rounded-lg bg-indigo-600 dark:bg-indigo-500 px-3 py-2 text-xs text-white shadow-sm transition hover:bg-indigo-500 dark:hover:bg-indigo-400"
                >
                  View details
                </button>
              </div>
